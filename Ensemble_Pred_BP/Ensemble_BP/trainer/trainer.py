...
class Trainer(BaseTrainer):
        ...
        for epoch in range(self.max_epoch_num):
            ...
            for idx, batch in enumerate(tbar):
                ...
                for seed_value in seed_list:
                    ...
                    BP_physformer, _, _, _ = self.model_physformer(combined_data, gra_sharp)
                    BP_physnet, _, _, _ = self.model_physnet(combined_data)
                    
                    RGB_BP_physformer, YUV_BP_physformer = torch.split(BP_physformer, RGB_data.size(0), dim=0)
                    RGB_BP_physnet, YUV_BP_physnet = torch.split(BP_physnet, RGB_data.size(0), dim=0)
    ...
    def save_model(self, index):
        ...
        print('Saved Model Path: ', self.new_dir)
