...
class convert_vid2npy():
    def __init__(self, config, path):
        ...
        self.subject_number = config['PREPROCESS']['SUBJECT_NUMBER']
    ...
    def vid2npy(self):
        ...
        self.process_all_subjects(data, self.vid_path)
    ...
    # 완료된 subject 목록을 로드 또는 초기화        
        with tqdm(total=len(data.keys()), desc="Processing Subjects", unit="subject") as pbar_subjects:
            ...
            start_subject = self.subject_number
            for subject in data.keys():
                subject_number = int(''.join(filter(str.isdigit, subject)))
            
            # subject 번호가 start_subject 이상일 때만 처리
                if subject_number < start_subject:
                    pbar_subjects.update(1)  # Progress bar를 업데이트
                    continue
                ...
                for video_path in video_paths:
                    ...
                    npy_index = self.process_video(video_path, subject_output_dir, target_size=(self.H,self.W), backend=self.backend, chunk_size=self.chunk_size,start_idx=npy_index,subject_name=subject)
        ...
        with tqdm(total=total_num, desc=f"Processing {subject_name}", unit="frame", leave=False) as pbar_frames:
            while True:
                ...
                if frame_count % chunk_size == 0:
                    ...
                    #print(f'RGB : {type(face_numpy)}')
                    
                    face_frames_np = self.diff_normalize_data(face_numpy)
                    #face_frames_np = self.standardized_data(face_frames)
                    # RGB npy 파일 저장
                    ...
                    # rint(f'yuv : {type(yuv_frames_np)}')
                    yuv_frames_np = self.diff_normalize_data(yuv_frames_np)
                    #yuv_frames_np = self.standardized_data(yuv_frames)
