...
class RegressionHead(nn.Module):
    ...
    def forward(self, x):
        h = torch.sigmoid(self.fc1(x.clone()))
        y = self.fc2(h.clone())
        return y
...
class ViT_ST_ST_Compact3_TDC_gra_sharp(nn.Module):
    ...
    def __init__(
        ...
        self.regression_head = RegressionHead(input_dim = 160, hidden_dim=64)
    ...
    def forward(self, x):
        ...
        rPPG_ = self.ConvBlockLast(features_last)    # x [B, 1, 160]
        
        rPPG_ = rPPG_.squeeze(1)
        
        rPPG = self.regression_head(rPPG_)
